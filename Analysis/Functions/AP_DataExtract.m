...
if ~iscell(Analysis.Properties.StateToZero)
    ...
    Photo=cell(length(Analysis.Properties.PhotoCh),length(Analysis.Properties.StateToZero));
    for ii=1:length(Analysis.Properties.StateToZero)
    StateToZero=Analysis.Properties.StateToZero{ii};
    TimeToZero=SessionData.RawEvents.Trial{1,thisTrial}.States.(StateToZero)(1);    

    %% Licks
    LickPort=Analysis.Properties.LickPort;
    try
        LickData{ii}=SessionData.RawEvents.Trial{1,thisTrial}.Events.(LickPort);
        LickData{ii}=LickData{ii}-TimeToZero;
    catch
        LickData{ii}=NaN;  
    end
    ...
    if Analysis.Properties.Photometry==1
        ...
        % DFF and zero
            ...
            Photo{ii,thisCh}(1,:)=Time;
            Photo{ii,thisCh}(2,:)=Data';
            Photo{ii,thisCh}(3,:)=100*DFF;
        end  
    else
            Photo=[];
    end
        ...
        % Timimg parameters 
            ...
            Wheel{ii}(1,:)=Time;
            Wheel{ii}(2,:)=DataWheel;
            Wheel{ii}(3,:)=DataWheelDistance;
