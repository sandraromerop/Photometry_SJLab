...
%% Timing
if ~iscell(DefaultParam.StateToZero)
    ...
    for ii = 1:length(DefaultParam.StateToZero)        
        handles.StateToZero{ii}     =handles.(DefaultParam.StateToZero{ii});
        for trialNb = 1:length(SessionData.RawEvents.Trial)
            ZeroTime{ii,trialNb}                    =SessionData.RawEvents.Trial{trialNb}.States.(handles.StateToZero{ii} )(1);
            handles.CueTime{ii,trialNb}             =SessionData.RawEvents.Trial{trialNb}.States.(handles.StateOfCue)-ZeroTime{ii,trialNb};
            handles.OutcomeTime{ii,trialNb}         =SessionData.RawEvents.Trial{trialNb}.States.(handles.StateOfOutcome)-ZeroTime{ii,trialNb};
            handles.CueTimeReset(ii,trialNb,:)        =DefaultParam.CueTimeReset;
            handles.OutcomeTimeReset(ii,trialNb,:)    =DefaultParam.OutcomeTimeReset;
        end
    end
end
%% Licks
